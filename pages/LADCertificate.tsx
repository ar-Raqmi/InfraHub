...
const LADCertificate: React.FC<LADCertificateProps> = ({ project, pjaUser, onClose }) => {
    ...
    const handleDownload = async () => {
        ...
        setTimeout(async () => {
            const element = document.getElementById('lad-cert-container');
            if (element) {
                const opt = {
                    margin: 0,
                    filename: `Perakuan_LAD_${project.noFail || 'Draft'}.pdf`,
                    image: { type: 'jpeg', quality: 0.98 },
                    html2canvas: { scale: 2, useCORS: true },
                    jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' } // Landscape based on width of text usually, but image looks Portrait. Let's stick to A4 Portrait as it looks stacked.
                };
                // Override to Portrait as image looks vertical
                opt.jsPDF = { unit: 'mm', format: 'a4', orientation: 'portrait' };
    ...
    const LabelRow = ({ label, value, boldValue = false }: { label: string, value: string | React.ReactNode, boldValue?: boolean }) => (
        <div className="flex text-[11px] mb-1.5 items-start">
            <div className="w-[200px] shrink-0 uppercase">{label}</div>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in" style={{ zIndex: 9999 }}>
            <div className="bg-white dark:bg-slate-900 w-full max-w-[230mm] h-[95vh] rounded-3xl flex flex-col shadow-2xl overflow-hidden relative animate-slide-up">
                ...
                <div className="flex-1 overflow-y-auto bg-gray-100 p-8 flex justify-center">
                    <div id="lad-cert-container" className="w-[210mm] min-h-[297mm] bg-white p-[20mm] shadow-lg text-black font-sans leading-snug relative box-border">
                        ...
                        <div className="border-2 border-black p-6 font-bold text-[12px] mb-8">
                            ...
                            {/* The Calculation Line from Image */}
                            {!isSmallProject ? (
