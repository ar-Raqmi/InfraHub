...
const BQPelarasanEditor: React.FC<BQPelarasanEditorProps> = ({ 
    ...
    const renderCalculationPartRow = (bill: BQGroup, item: BQItem, part: CalculationPart) => {
        ...
        const inputClass = isGlobal || readOnly ? `${inputClassBase} bg-transparent text-slate-400 cursor-not-allowed` : `${inputClassBase} bg-yellow-50/50  text-yellow-700  border-b border-yellow-300  rounded-sm`;
        return (
          <div key={part.id} className="flex flex-wrap items-center gap-2 text-xs bg-white  p-1.5 rounded-lg border border-slate-200  mb-1 last:mb-0">
              <input type="text" value={part.label || ''} onChange={(e) => updateCalculationPart(bill.id, item.id, part.id, { label: e.target.value })} disabled={readOnly} className="w-16 bg-transparent border-b border-dashed border-slate-300  focus:border-yellow-500 outline-none text-slate-500 placeholder-slate-400 text-[10px]" placeholder="Label" />
                              <div className={`flex items-center gap-1 rounded px-1.5 py-0.5 border transition-colors ${part.hasLength ? 'bg-yellow-50 border-yellow-200' : 'bg-transparent border-transparent opacity-60'}`}>
              <span className="text-[10px] font-bold text-slate-500">P</span>{part.hasLength && (<DimensionInput value={part.length || 0} onChange={val => updateCalculationPart(bill.id, item.id, part.id, { length: val })} className={inputClass} disabled={isGlobal || readOnly} />)}</div>
              {part.hasLength && (part.hasWidth || part.hasDepth) && <span className="text-slate-300">×</span>}
                              <div className={`flex items-center gap-1 rounded px-1.5 py-0.5 border transition-colors ${part.hasWidth ? 'bg-yellow-50 border-yellow-200' : 'bg-transparent border-transparent opacity-60'}`}>
              <span className="text-[10px] font-bold text-slate-500">L</span>{part.hasWidth && (<DimensionInput value={part.width || 0} onChange={val => updateCalculationPart(bill.id, item.id, part.id, { width: val })} className={inputClass} disabled={isGlobal || readOnly} />)}</div>
              {part.hasWidth && part.hasDepth && <span className="text-slate-300">×</span>}
                              <div className={`flex items-center gap-1 rounded px-1.5 py-0.5 border transition-colors ${part.hasDepth ? 'bg-yellow-50 border-yellow-200' : 'bg-transparent border-transparent opacity-60'}`}>
              <span className="text-[10px] font-bold text-slate-500">T</span>{part.hasDepth && (<DimensionInput value={part.depth || 0} onChange={val => updateCalculationPart(bill.id, item.id, part.id, { depth: val })} className={inputClass} disabled={isGlobal || readOnly} />)}</div>
              <span className="text-slate-300">×</span>
              <div className={`flex items-center gap-1 rounded px-1.5 py-0.5 border border-transparent ${part.multiplier !== 1 ? 'bg-orange-50  border-orange-200' : 'opacity-60'}`}><DimensionInput value={part.multiplier || 1} onChange={val => updateCalculationPart(bill.id, item.id, part.id, { multiplier: val })} disabled={readOnly} className="w-8 bg-transparent outline-none text-center font-bold text-slate-700  placeholder-slate-400" placeholder="1" /></div>
    ...
    const renderItemRow = (bill: BQGroup, item: BQItem, index: number, isHidden: boolean) => {
        ...
        if (diff > 0.01) colorClass ="text-blue-600  font-bold"; else if (diff < -0.01) colorClass ="text-red-600  font-bold";

        if (item.type === 'HEADER') {
            const isLevel0 = hierarchyLevel === 0;
            return (
                                  <div key={item.id} className={`flex items-center gap-2 py-3 border-b border-slate-100 ${isLevel0 ? 'bg-slate-100' : 'bg-slate-50/50'} px-4 -mx-4`}>
                    ...
                    <button onClick={() => toggleCollapse(bill.id, item.id)} className="p-1 rounded hover:bg-slate-200  text-slate-400 transition-colors">{item.isCollapsed ? <ChevronRight className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}</button>
                                          <span className={`text-sm ${isLevel0 ? 'font-bold uppercase text-slate-800' : 'font-semibold text-slate-700 pl-1'}`}>{item.description}</span>
                    
                </div>
            );
        }
        return (
            <div key={item.id} className="py-4 border-b border-slate-100  last:border-0 hover:bg-slate-50  px-2 rounded-xl transition-colors duration-300">
                <div className="flex flex-col xl:flex-row gap-4">
                    <div className="flex-1 min-w-0">
                         <div className="flex gap-3 mb-2">
                             <div className="text-xs font-black text-slate-400 mt-1 min-w-[30px]">{autoNumber}</div>
                             <div className="flex-1 pl-10"><p className="text-sm font-medium text-slate-800  leading-relaxed">{item.description}</p>{item.variant && <p className="text-xs text-slate-500 italic mt-1">{item.variant}</p>}<div className="mt-2 flex items-center gap-2"><span className="text-xs bg-slate-100  px-2 py-0.5 rounded text-slate-500 font-mono">{item.unit}</span><span className="text-xs text-slate-400">@ {formatCurrency(item.rate)}</span></div></div>
                         </div>
                         <div className="ml-12 bg-yellow-50/30  p-3 rounded-lg border border-yellow-100">
                             <div className="flex items-center gap-2 mb-2"><div className="text-[10px] font-bold text-yellow-600  uppercase tracking-wider flex items-center gap-1"><Calculator className="w-3 h-3" /> Pelarasan (Adjusted)</div><button onClick={() => toggleGlobal(bill.id, item.id)} disabled={readOnly} className={`p-1 rounded-md transition-colors border text-[10px] flex items-center gap-1 ${item.isGlobal ? 'bg-emerald-100 text-emerald-600 border-emerald-200' : 'bg-white text-slate-400 border-slate-200'}`}>{item.isGlobal ? <Link className="w-3 h-3" /> : <Unlink className="w-3 h-3" />}{item.isGlobal ? 'Linked' : 'Manual'}</button></div>
                             <div className="space-y-1">{(item.calculationParts || []).map(part => renderCalculationPartRow(bill, item, part))}</div>
                             <div className="flex items-center justify-between mt-2 pt-2 border-t border-yellow-200">{!readOnly && (<button onClick={() => addCalculationPart(bill.id, item.id)} className="text-[10px] flex items-center gap-1 text-yellow-600 hover:text-yellow-700 font-bold px-2 py-1 rounded hover:bg-yellow-100"><PlusCircle className="w-3 h-3" /> Tambah</button>)}<div className="flex items-center gap-2 ml-auto"><span className="text-xs text-slate-400">Qty Laras:</span><DimensionInput value={item.qty} onChange={(val) => updateItem(bill.id, item.id, { qty: val })} disabled={readOnly} className="w-20 text-right bg-white  border border-slate-200  rounded px-2 py-1 text-sm font-bold text-slate-700" /></div></div>
                         </div>
                    </div>
                    <div className="w-full xl:w-[320px] shrink-0 bg-slate-50  rounded-xl border border-slate-200  p-3 text-xs">
                         <div className="grid grid-cols-2 gap-x-4 gap-y-2"><div className="col-span-2 border-b border-slate-200  pb-1 mb-1 font-bold text-slate-400 uppercase tracking-widest text-[10px]">Perbandingan</div><div className="text-slate-500">Asal (Qty)</div><div className="text-right font-mono text-slate-600">{originalQty}</div><div className="text-slate-500">Asal (RM)</div><div className="text-right font-mono text-slate-600">{formatCurrency(originalAmount)}</div><div className="col-span-2 border-b border-slate-200  my-1"></div><div className="text-slate-500 font-bold">Laras (Qty)</div><div className="text-right font-mono font-bold text-slate-800">{item.qty}</div><div className="text-slate-500 font-bold">Laras (RM)</div><div className="text-right font-mono font-bold text-slate-800">{formatCurrency(item.amount)}</div><div className="col-span-2 border-b border-slate-200  my-1"></div><div className="font-bold">Beza (RM)</div><div className={`text-right font-mono ${colorClass}`}>{diff > 0 ? '+' : ''}{formatCurrency(diff)}</div></div>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        ...
        <div className="flex flex-col md:flex-row gap-4 items-start">
            <div className="w-full md:w-72 flex flex-col gap-2 shrink-0 sticky top-24 max-h-[calc(100vh-8rem)]">
                 <div className="bg-yellow-50  p-3 rounded-xl border border-yellow-100  flex items-center justify-between"><span className="text-xs font-bold uppercase text-yellow-700  tracking-wider">Navigasi Pelarasan</span></div>
            ...
            <div className="flex-1 w-full min-w-0 bg-white  rounded-xl shadow-inner flex flex-col">
                {activeBill ? (
                    <>
                        <div className="p-4 border-b border-slate-100  bg-slate-50/50  sticky top-20 z-20"><div className="flex items-center justify-between mb-2"><h2 className="text-lg font-bold text-slate-800  uppercase">{activeBill.title}</h2><div className="text-right text-xs text-slate-400 shrink-0 ml-4">Pelarasan Total: <span className="text-yellow-600 font-bold text-sm">{formatCurrency(activeBill.items.reduce((s,i) => s + (i.amount||0), 0))}</span></div></div>
                             {activeBill && (
                                                                  <div className={`mt-2 p-3 rounded-xl border transition-colors ${isDimsDirty ? 'bg-orange-50 border-orange-200' : 'bg-yellow-50/50 border-yellow-100'}`}>
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                                        <div className="flex items-center gap-2">
                                            <div className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase">
                                                ...
                                                {activeBill.calculationId && !globalCalculationsPelarasan?.[activeBill.calculationId] && projectData.globalCalculations?.[activeBill.calculationId] && (
                                                    <span className="text-[9px] text-emerald-500 bg-emerald-50  px-1.5 py-0.5 rounded ml-1">
                                                        (Sync from BQ)
                                                    </span>
                                                )}
                                            </div>
                                        ...
                                        <div className="flex items-center gap-2 w-full sm:w-auto">
                                            ...
                                            {isDimsDirty && !readOnly && (<button onClick={handleSaveGlobalDims} className="ml-2 px-3 py-1.5 bg-yellow-600 hover:bg-yellow-700 text-white text-xs font-bold rounded-lg shadow-sm flex items-center gap-1 transition-colors"><Save className="w-3 h-3" /> Kemaskini</button>)}
                                        </div>
                                    </div>
                                    {pelarasanData.filter(b => b.calculationId === activeBill.calculationId).length > 1 && (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8"><Layers className="w-16 h-16 mb-4 opacity-20" /><h3 className="text-lg font-bold text-slate-600">Tiada Senarai Dipilih</h3><p className="text-sm max-w-xs text-center mt-2">Pilih senarai dari navigasi sebelah kiri untuk membuat pelarasan.</p></div>
                )}
            </div>
        </div>
                      ...
                      {pelarasanData
                        ...
                        .map(b => (
                          <button 
                            ...
                            className="w-full text-left p-4 rounded-2xl border border-slate-100  bg-slate-50/50  hover:border-yellow-500 hover:bg-yellow-50  transition-colors group"
                          >
                              <div className="flex items-center justify-between">
                                  ...
                                  <ChevronRight className="w-4 h-4 text-slate-300 group-hover:text-yellow-500 transition-colors" />
                              </div>
                          </button>
                      ))}
