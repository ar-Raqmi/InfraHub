                      ...
                      <div className="space-y-8 animate-fade-in">
                          <div className="bg-slate-50  p-6 rounded-3xl border border-slate-100  shadow-inner">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                  <div><label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 pl-1">Pilih Lokasi</label><select value={templateLocation} onChange={(e) => setTemplateLocation(e.target.value)} className={`w-full px-4 py-3 rounded-xl bg-white  border-2 outline-none transition-colors font-bold text-sm ${templateError && !templateLocation ? 'border-red-400 animate-shake' : 'border-slate-100  focus:border-emerald-500'}`}><option value="">-- Pilih Lokasi --</option>{locationRows.map(row => <option key={row.id} value={row.id}>{row.lokasi || '(Tiada Nama Lokasi)'}</option>)}</select>{templateError && !templateLocation && <p className="text-[10px] text-red-500 font-bold mt-1 ml-1 flex items-center gap-1"><AlertTriangle className="w-3 h-3" /> Lokasi diperlukan.</p>}</div>
