...
// (getColorStyles, toRoman, AutoResizeTextarea, getItemLevel, getAutoNumber
        ...
        const nextBills = (initialData || []).map(bill => ({
            ...
            items: (bill.items || []).map(item => ({
                ...
                calculationParts: (item.calculationParts || []).map(part => ({
                    ...
                    multiplier: part.multiplier == null ? 1 : Number(part.multiplier)
                }))
            }))
        }));
                        ...
                        <div className="flex items-center gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
                            {isLevel0 && (
                                <button
                                    ...
                                    className="text-[10px] bg-white border border-slate-200 hover:border-emerald-500 rounded px-2 py-1 text-slate-500 hover:text-emerald-600 font-bold transition-colors mr-1 flex items-center gap-1 shrink-0"
                                    title="Pindah ke Bil No. lain"
                                >
                                    <FolderOpen className="w-3.5 h-3.5" /> Pindah
                                </button>
                            )}
                            <button onClick={() => moveItem(bill.id, item.id, 'up')} disabled={index === 0} className="text-slate-400 hover:text-emerald-500 p-1 rounded hover:bg-emerald-50  disabled:opacity-30"><ChevronUp className="w-4 h-4" /></button>
                            <button onClick={() => moveItem(bill.id, item.id, 'down')} className="text-slate-400 hover:text-emerald-500 p-1 rounded hover:bg-emerald-50  disabled:opacity-30"><ChevronDown className="w-4 h-4" /></button>
                    ...
                    <div className="flex flex-col items-end gap-2 w-full sm:w-auto border-t sm:border-t-0 border-slate-200  pt-2 sm:pt-0 pl-2 border-l-0 sm:border-l">
                        ...
                        {!readOnly && (
                            <div className="mt-auto flex items-center gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => moveItem(bill.id, item.id, 'up')} disabled={index === 0} className="p-1 text-slate-300 hover:text-emerald-500 hover:bg-emerald-50  rounded-lg transition-colors disabled:opacity-30"><ChevronUp className="w-4 h-4" /></button>
                                <button onClick={() => moveItem(bill.id, item.id, 'down')} className="p-1 text-slate-300 hover:text-emerald-500 hover:bg-emerald-50  rounded-lg transition-colors disabled:opacity-30"><ChevronDown className="w-4 h-4" /></button>
                                <button onClick={() => requestDeleteItem(bill.id, item, index)} className="p-1 text-slate-300 hover:text-red-500 hover:bg-red-50  rounded-lg transition-colors"><Trash2 className="w-4 h-4" /></button>
                            </div>
                        )}
                    </div>
                </div>
